      ...
      <div class="developer_detail_contents">
        ...
        <ul class="detail">
          ...
          <li>
            <p class="github_net">
              <span>Github地址： </span>
              <span @click="linkClick">{{ link }}</span>
            </p>
          </li>
          <li>
            <div class="registration_task3" v-if="status == 1">
              <img
                src="@assets/images/developerdetail/矢量智能对象.png"
              />已报名
            </div>
            <div class="registration_task2" v-else-if="status == 2">
              <img src="@assets/images/developerdetail/完成.png" />任务结束
            </div>
        ...
        <div class="overhide" v-if="marqueetrueall">
          <ul v-if="marqueetrue">
            <li v-for="(item, index) in newsList" :key="index">
              <span>{{ item.name }}</span
              >完成了该赏金任务，获得了<span>{{ item.amount }}</span
              >UENC
            </li>
            &nbsp;&nbsp;&nbsp;&nbsp;
          </ul>
